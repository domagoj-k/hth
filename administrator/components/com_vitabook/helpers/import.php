...
abstract class VitabookHelperImport
{
    ...
    */  
    static function replaceSmilies($text)
    {
        ...
        {
            $message = str_replace($match, '[codetemp'.$i.']', $text);
            $i++;
        }

        foreach($smiley as $i => $sm)
        {
            $text = str_replace($i, '<img src="'.JUri::root().'components/com_vitabook/assets/vitabookemoticons/img/smilies/'.$sm.'" alt="" border="0" class="smiley" />', $text);
        }
