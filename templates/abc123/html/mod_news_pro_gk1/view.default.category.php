...
<?php if($news_amount > 0) : ?>
	...
	<div class="gk_npro_mainwrap<?php if($this->config['autoanim'] == TRUE) echo ' autoanim'; ?>" id="gk_npro-<?php echo $this->config['module_unique_id']; ?>" <?php if($block_width != 0) : ?>style="width: <?php echo $block_width; ?>;"<?php endif; ?>>
		...
		<div class="gk_npro_wrap1" <?php if($block_width != 0) : ?>style="width: <?php echo $block_width; ?>;"<?php endif; ?>>
			<div class="gk_npro_wrap2" style="width: 10000px;">
				<?php for($n = 0; $n < count($this->content['ID']); $n++) : ?>
				<div class="gk_npro_block_wrap" <?php if($block_width != 0) : ?>style="width: <?php echo $block_width; ?>;"<?php endif; ?>>
					...
					<div class="gk_npro_full" style="<?php if($module_width != 0) : ?>width: <?php echo $module_width; ?>;<?php endif; ?>float: <?php echo $module_float; ?>;">
						
						<?php if($this->config['show_category'] == 1) : ?>
						<h3 class="gk_npro_cat_header" id="gk_npro-<?php echo $this->config['module_unique_id']; ?>-header-<?php echo $n; ?>">
							...
							<table class="gk_npro_table" <?php if($module_width != 0) : ?>style="width: <?php echo $module_width; ?>;"<?php endif; ?>>
								...
								<tr valign="top">
									<?php for($k = 0; $k < $this->config['news_column']; $k++) : ?>		
										<?php if((($j * $this->config['news_column']) + $k) >= count($news_html_tab[$n])) : ?>
										...
										<?php else : ?>
											...
											<div class="nspro_bg_wrap"><?php if(isset($news_html_tab[$n][(($j * $this->config['news_column']) + $k)])) echo $news_html_tab[$n][(($j * $this->config['news_column']) + $k)];?></div>	 
											</td>		
										<?php endif; ?>
									<?php endfor; ?>
								</tr>
