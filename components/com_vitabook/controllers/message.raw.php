...
class VitabookControllerMessage extends JControllerForm
{
    ...
    public function save($key = null, $urlVar = null)
    {
        ...
        if (JFactory::getUser()->get('guest'))
        {
            if (JComponentHelper::getParams('com_vitabook')->get('guest_email_activation')){
                jexit(json_encode(array("state"=>2, "result" => JText::_( 'COM_VITABOOK_MESSAGE_WAIT_ACTIVATE' ) )));
            }            
   ...
   /**
    ...
    {
        // Check for request forgeries.
        VitabookHelperMail::checkMailHash(JRequest::getInt('messageId'),'activate') or jexit(json_encode(array("success"=>0, "state" => JText::_('JINVALID_TOKEN')))); // nog een redirect met invalid token waarschuwing
        ...
        else
        {
            ...
            if($message->published){
                $this->setRedirect(JRoute::_('index.php?option=com_vitabook&messageId='.$messageId, true, -1).'#'.$messageId, JText::_('COM_VITABOOK_MESSAGE_ACTIVATED_PUBLISHED'));
            }            
            else{
                $this->setRedirect(JRoute::_('index.php?option=com_vitabook&messageId='.$messageId, true, -1).'#'.$messageId, JText::_('COM_VITABOOK_MESSAGE_ACTIVATED_UNPUBLISHED'));
            }
        }
    } 
