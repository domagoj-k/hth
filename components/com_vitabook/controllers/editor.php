...
class VitabookControllerEditor extends JControllerLegacy
{
	   ...
	   ?> 
		...
		<html>
			<head>
				...
				<link rel="stylesheet" href="<?php echo JURI::root() . "components/com_vitabook/assets/vitabookupload/css/vitabookupload.css"; ?>" type="text/css" />
				...
				<script type="text/javascript">
					...
					<?php if (JFactory::getUser()->authorise('vitabook.upload.image', 'com_vitabook')) { ?>
						function startUpload()
						{
							if(document.getElementById('image').value != '') {
								document.getElementById('upload_process').innerHTML = '<img src="<?php echo JURI::root() . "components/com_vitabook/assets/vitabookupload/img/loader.gif"; ?>" alt="loading" />';
				    ...
				    {
						if(document.getElementById('url').value != '') {
							document.getElementById('insert_process').innerHTML = '<img src="<?php echo JURI::root() . "components/com_vitabook/assets/vitabookupload/img/loader.gif"; ?>" alt="loading" />';
							document.getElementById('insert-button').disabled=true;
	...
	/**
	 * Show the VitabookVideo plugin content
	 *
	 */
	...
	?>
		...
		<head>
			...
			<script type="text/javascript">
				...
				{
					var videoSize = 'width="350px" height="300px"', domSize = 'width:350px;" height:300px;', options = '';
					...
					if(url.match(/http:\/\/(www.)?youtube|youtu\.be/i))
					{
						...
						if (match&&match[1].length==11) {
							...
							var	objectCode = '<iframe src="'+youtubeUrl+'?wmode=opaque" '+videoSize+' frameborder="0"></iframe>';
					...
					else if(url.match(/http:\/\/(player.)?vimeo\.com/i))
					{
						...
						if (match) {
							...
							var	objectCode = '<iframe src="'+vimeoUrl+'" '+videoSize+' frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
